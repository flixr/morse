...
class RotorcraftWaypointActuatorClass(morse.core.actuator.MorseActuatorClass):
    ...
    def __init__(self, obj, parent=None):
        ...
        self.add_property('_v_pgain', 5, 'VerticalPgain')
        self.add_property('_v_dgain', 5, 'VerticalDgain')
        ...
        self.nominal_thrust = self.robot_parent.blender_obj.mass * 9.8
        logger.info("nominal thrust: %.3f", self.nominal_thrust)
        self._att_comp_limit = math.cos(self._max_bank_angle) ** 2
    ...
    def default_action(self):
        ...
        att_comp = max(self._att_comp_limit, math.cos(roll) * math.cos(pitch))
        thrust_ff = self.nominal_thrust / att_comp
